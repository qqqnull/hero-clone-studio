...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { countryId, count = 100 } = await req.json()
    ...
    console.log(`Generated ${newNumbers.length} phone numbers for country ${country.code}`)
    ...
    return new Response(
      JSON.stringify({
        ...
        message: `Successfully generated ${newNumbers.length} phone numbers for ${country.code}`
      }),
